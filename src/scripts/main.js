document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-donation]').forEach(option => option.addEventListener('click', () => {
    document.querySelectorAll('[data-donation]').forEach(item => item.classList.remove('selected'));
    option.classList.add('selected');
    const currency = document.getElementById('currency-selector')?.value || 'USD';
    const symbol = currency === 'USD' ? '$' : '€';
    document.querySelector('[data-donation-amount]').textContent = `${symbol}${option.dataset.donation}`;
    
    // Show payment methods
    const paymentMethods = document.getElementById('payment-methods');
    const paymentInfo = document.getElementById('payment-info');
    if (paymentMethods && paymentInfo) {
  ...
  if (currencySelector) {
    currencySelector.addEventListener('change', () => {
      ...
      document.querySelectorAll('.donation-btn').forEach(btn => {
        const amount = btn.dataset.donation;
        btn.textContent = `${symbol}${amount}`;
      });
      
      // Update selected amount display
      const selectedAmount = document.querySelector('[data-donation-amount]');
      if (selectedAmount && selectedAmount.textContent !== 'Choose an amount') {
        const selectedBtn = document.querySelector('.donation-btn.selected');
        if (selectedBtn) {
          selectedAmount.textContent = `${symbol}${selectedBtn.dataset.donation}`;
        }
      }
    });
  }
  ...
  document.querySelectorAll('.payment-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const currency = document.getElementById('currency-selector')?.value || 'USD';
      const selectedAmount = document.querySelector('.donation-btn.selected')?.dataset.donation || '0';
      
      // Here you would integrate with actual payment processors
      // For now, show an alert with the selection
      alert(`Payment method: ${method.toUpperCase()}\nAmount: ${currency === 'USD' ? '$' : '€'}${selectedAmount}\n\nPayment integration will be configured with your API keys in .env`);
      
      // TODO: Integrate with PayPal/Stripe when API keys are configured
      // Example: window.location.href = `/api/payment/${method}?amount=${selectedAmount}&currency=${currency}`;
    });
  });
  ...
  const translations = {
    ...
    "fr": {
      ...
      "payment": {"eyebrow":"Soutenir la mission","title":"Votre générosité fait la différence.","description":"Chaque don nous aide à servir avec soin, à équiper les jeunes et à renforcer les communautés.","choose_amount":"Choisir un montant","select_amount":"Sélectionnez un montant qui vous convient.","currency":"Devise:","selected_amount":"Montant sélectionné:","choose_amount_placeholder":"Choisir un montant","choose_payment":"Choisir le mode de paiement","select_payment":"Sélectionnez votre mode de paiement préféré:","paypal":"PayPal","paypal_desc":"Payer avec un compte PayPal","card":"Carte de crédit/débit","card_desc":"Visa, Mastercard, etc.","arrange_gift":"Organiser votre don","select_amount_instruction":"Sélectionnez un montant ci-dessus pour choisir votre mode de paiement.","thank_you":"Merci de vous associer à nous.","giving_enables":"Votre don permet des soins pratiques et un encouragement rempli de foi là où c'est le plus nécessaire."},
      "footer": {"copyright":"© 2026 Nibature International Ministries. Tous droits réservés."}
    },
    "sv": {
      "nav": {"home":"Hem","about":"Om oss","services":"Vårt arbete","blog":"Berättelser","gallery":"Galleri","contact":"Kontakt","give":"Ge"},
      "home": {"eyebrow":"Tro i handling","hero_title":"Hopp som sträcker sig bortom söndagen.","hero_description":"Nibature International Ministries går bredvid människor och samhällen genom tro, omsorg, lärande och praktiskt stöd.","discover_work":"Upptäck vårt arbete","support_mission":"Stöd uppdraget","mission_title":"Vårt uppdrag","mission_subtitle":"Växande tro. Återställande av värdighet. Förstärkning av samhällen.","mission_description":"Vi tror att varaktig förändring börjar när människor ses, hörs och utrustas. Vårt tjänst skapar utrymme för andlig tillväxt och praktiska möjligheter.","meet_ministry":"Träffa vårt tjänst","what_we_do":"Vad vi gör","serving_people":"Tjäna människor med medkänsla","every_programme":"Varje program är utformat för att föra människor samman och flytta samhällen framåt.","community_outreach":"Samhällsengagemang","community_outreach_desc":"Praktisk vård, familjestöd och andlig uppmuntran för människor i behov.","youth_empowerment":"Ungdoms empowerment","youth_empowerment_desc":"Mentorskap, färdigheter och ledarskapsmöjligheter för nästa generation.","faith_counselling":"Tro och rådgivning","faith_counselling_desc":"Medkännande vägledning för individer och familjer på livets resa.","together_we_can":"Tillsammans kan vi","make_room":"Gör plats för hopp.","join_community":"Gå med i ett community engagerat att tjäna med generositet, integritet och kärlek.","get_involved":"Engagera dig"},
      ...
      "payment": {"eyebrow":"Stöd uppdraget","title":"Din generositet gör skillnad.","description":"Varje gåva hjälper oss att tjäna med omsorg, utrusta unga människor och stärka samhällen.","choose_amount":"Välj ett gåvobelopp","select_amount":"Välj ett belopp som känns rätt för dig.","currency":"Valuta:","selected_amount":"Valt belopp:","choose_amount_placeholder":"Välj ett belopp","choose_payment":"Välj betalningsmetod","select_payment":"Välj din föredragna betalningsmetod:","paypal":"PayPal","paypal_desc":"Betala med PayPal-konto","card":"Kredit-/betalkort","card_desc":"Visa, Mastercard, etc.","arrange_gift":"Organisera din gåva","select_amount_instruction":"Välj ett belopp ovan för att välja din betalningsmetod.","thank_you":"Tack för att du samarbetar med oss.","giving_enables":"Ditt givande möjliggör praktisk vård och trofylld uppmuntran där det behövs mest."},
